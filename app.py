...
# ── MARKET DATA ───────────────────────────────────────────────────────────────
SYMBOL_MAP = {
    "BTC":"BTCUSDT","NAS100":"NQ=F","US30":"YM=F","SPX":"ES=F",
...
def market_data():
    ...
    try:
        ...
        r = requests.get("https://query1.finance.yahoo.com/v8/finance/chart/"+yahoo_sym,
        ...
        for i in range(len(ts)):
            ...
            candles.append({"t":ts[i]*1000,"o":quote.get("open",[])[i] or c,
                "h":quote.get("high",[])[i] or c,"l":quote.get("low",[])[i] or c,
                "c":c,"v":quote.get("volume",[])[i] or 0})
        return jsonify({"candles":candles,"instrument":instrument,"interval":interval})
    except Exception as e:
        return jsonify({"error":str(e)}), 500

# ── POLYMARKET: FETCH BY SLUG ─────────────────────────────────────────────────
