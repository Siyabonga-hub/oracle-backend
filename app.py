...
def fetch_by_slug(slug):
    # Try the direct slug endpoint first (more reliable)
    try:
        r = requests.get(GAMMA + "/events/slug/" + slug, headers=HEADERS, timeout=10)
        if r.status_code == 200:
            event = r.json()
            if isinstance(event, dict) and event.get("markets"):
                # Return first market from event
                return event["markets"][0]
    except Exception:
        pass
    # Fallback to search by slug param
...
def polymarket_odds():
    ...
    try:
        ...
        for offset in [0, -300, -600, 300]:
            ...
            # Note: BTC 5M slugs DO use timestamps, only weather slugs omit year
            try:
                m = fetch_by_slug(slug)
                if m:
    ...
    try:
        ...
        for term in terms:
            ...
            try:
                ...
                continue
        result = [parse_market(m) for m in markets[:15] if isinstance(m, dict)]
